@echo off
echo ========================================
echo   PUSH API - AJOUT CONTRACT END DATE
echo ========================================
...
powershell -Command "(Get-Content VERSION.md) -replace 'Version : .*', 'Version : 1.0.5' -replace 'Date : .*', 'Date : %date:~-4,4%/%date:~-7,2%/%date:~-10,2%' -replace 'Heure : .*', 'Heure : %time:~0,2%:%time:~3,2%:%time:~6,2%' -replace 'Commit : .*', 'Commit : Ajout contractEndDate et correction validation Planning' -replace 'Statut : .*', 'Statut : ⏳ En cours de deploiement' | Set-Content VERSION.md"

echo 6. Commit des modifications...
git commit -m "Version 1.0.5 - Ajout contractEndDate et correction validation Planning - Date: %date:~-4,4%/%date:~-7,2%/%date:~-10,2% %time:~0,2%:%time:~3,2%"
