@echo off
echo ========================================
echo   PUSH API - CORRECTION CONTRAINTES
echo ========================================
...
powershell -Command "(Get-Content VERSION.md) -replace 'Version : .*', 'Version : 1.0.8' -replace 'Date : .*', 'Date : %date:~-4,4%/%date:~-7,2%/%date:~-10,2%' -replace 'Heure : .*', 'Heure : %time:~0,2%:%time:~3,2%:%time:~6,2%' -replace 'Commit : .*', 'Commit : Correction contraintes planning - alignement enum' -replace 'Statut : .*', 'Statut : ⏳ En cours de deploiement' | Set-Content VERSION.md"

echo 6. Commit des modifications...
git commit -m "Version 1.0.8 - Correction contraintes planning - alignement enum - Date: %date:~-4,4%/%date:~-7,2%/%date:~-10,2% %time:~0,2%:%time:~3,2%"
