...
// Parser mots_de_passe.bat (pwd_NOM=... ou pwd_NOM_PRENOM=... pour homonymes)
function chargerNomsDepuisMotsDePasse(cheminFichier) {
  ...
  // Clés composées (NOM_PRENOM) et noms longs d'abord
  noms.sort((a, b) => {
...
function trouverNomParListe(texte, listeNoms) {
  ...
  for (const key of listeNoms) {
    ...
    if (parts.length >= 2) {
      // Homonyme: pwd_POUILLAUDE_LAURA → exiger NOM + PRENOM dans le PDF
    ...
    // Clé courte: ignorer s'il existe des homonymes NOM_PRENOM pour ce nom
    const hasCompounds = listeNoms.some(
      (n) => normalizeAccents(n).startsWith(`${keyNorm}_`)
    );
    if (hasCompounds) continue;

    const nomEsc = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const regex1 = new RegExp('\\b' + nomEsc + '\\b', 'i');
    if (regex1.test(texte)) return key;
    const regex2 = new RegExp('(?:Madame|Monsieur)[A-Za-zÀ-ÿ]*' + nomEsc, 'i');
    if (regex2.test(texte)) return key;
    if (keyNorm.length >= 3 && texteNorm.includes(keyNorm)) return key;
  }
  return null;
}
