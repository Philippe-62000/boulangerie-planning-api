...
for %%F in (*.pdf) do (
    ...
    REM Format attendu: YYYYMM Prenom NOM.pdf (dernier token = nom de famille)
    set "nom="
    REM Utiliser PowerShell pour extraire le dernier token et le convertir en majuscules
    for /f "delims=" %%N in ('powershell -Command "$parts = '!filename!' -split ' '; if ($parts.Count -gt 0) { ($parts[$parts.Count-1]).ToUpper() }"') do set "nom=%%N"
    
    REM Si PowerShell a échoué, utiliser une méthode de repli (token 2)
    if "!nom!"=="" (
    ...
    if "!pwd_trouve!"=="1" (
        REM Générer le nom du fichier protégé (ajouter _Protege avant l'extension)
        set "newname=!filename!_Protege.pdf"
