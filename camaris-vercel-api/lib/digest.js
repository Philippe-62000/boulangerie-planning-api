...
const { CamarisTerritoryEvent } = require('./models');
...
const getDidYouKnow = (d = new Date()) => {
  ...
  if (!didYouKnowCache.length) {
    return {
      type: 'didYouKnow',
      title: 'Le savez-vous ?',
      text: 'Nos artisans préparent vos produits chaque matin.'
    };
  }
  const idx = (d.getFullYear() * 366 + d.getMonth() * 31 + d.getDate()) % didYouKnowCache.length;
  return { type: 'didYouKnow', title: 'Le savez-vous ?', text: didYouKnowCache[idx] };
};
