@echo off
echo ========================================
echo   PUSH API - FIX PLANNING COMPLET
echo ========================================
...
echo Version : 1.1.6 > VERSION.md
echo Date : %date:~-4,4%/%date:~-7,2%/%date:~-10,2% >> VERSION.md
echo Heure : %time:~0,2%:%time:~3,2%:%time:~6,2% >> VERSION.md
echo Commit : Fix complet planning avec toutes les fonctions >> VERSION.md
echo Statut : ⏳ En cours de deploiement >> VERSION.md

echo 6. Commit des modifications...
git commit -m "Version 1.1.6 - Fix complet planning avec toutes les fonctions - Date: %date:~-4,4%/%date:~-7,2%/%date:~-10,2% %time:~0,2%:%time:~3,2%"
