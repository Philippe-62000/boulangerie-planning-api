...
echo Version : 1.1.1 > VERSION.md
echo Date : %date:~-4,4%/%date:~-7,2%/%date:~-10,2% >> VERSION.md
echo Heure : %time:~0,2%:%time:~3,2%:%time:~6,2% >> VERSION.md
echo Commit : Fix planning generation constraints >> VERSION.md
echo Statut : ⏳ En cours de deploiement >> VERSION.md

echo 6. Commit des modifications...
git commit -m "Version 1.1.1 - Fix planning generation constraints - Date: %date:~-4,4%/%date:~-7,2%/%date:~-10,2% %time:~0,2%:%time:~3,2%"
