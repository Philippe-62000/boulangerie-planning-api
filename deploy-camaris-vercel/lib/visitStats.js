const { CamarisVisitDaily, CamarisVisitCounter } = require('./models');
const { getIsoWeekKey } = require('./digest');
...
async function recordVisit(siteKey = SITE_LON) {
  const dateKey = getParisDateKey();
  const weekKey = getParisWeekKey();
  const monthKey = getParisMonthKey();

  await CamarisVisitDaily.findOneAndUpdate(
    { siteKey, dateKey },
    { $inc: { count: 1 }, $setOnInsert: { weekKey, monthKey } },
    { upsert: true, new: true }
  );

  await CamarisVisitCounter.findOneAndUpdate(
    { siteKey },
    { $inc: { totalVisits: 1 } },
    { upsert: true }
  );

  return getVisitSummary(siteKey);
}
